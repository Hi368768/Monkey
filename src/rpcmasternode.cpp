...
void SendMoney(const CTxDestination &address, CAmount nValue, CWalletTx& wtxNew, AvailableCoinsType nCoinType = ALL_COINS)
{
    // Check amount
    if (nValue <= 0)
    ...
    if (!pwalletMain->CreateTransaction(scriptPubKey, nValue, wtxNew, reservekey, nFeeRequired, strError, NULL, nCoinType))
    {
...
Value darksend(const Array& params, bool fHelp)
{
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "darksend <MONKaddress> <amount>\n"
            "MONKaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "darksend <MONKaddress> <amount>\n"
            "MONKaddress, denominate, or auto (AutoDenominate)"
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        ...
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) Monkey address of current masternode\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        (strCommand != "count" && strCommand != "current" && strCommand != "debug" && strCommand != "genkey" && strCommand != "list" && strCommand != "list-conf"
        ...
        throw runtime_error(
                ...
                "\nAvailable commands:\n"
                "  connect      - Attempts to connect to specified masternode address\n"
                "  count        - Print number of all known masternodes\n"
                "  current      - Print info on current masternode winner\n"
                "  debug        - Print masternode status\n"
                "  genkey       - Generate new masternodeprivkey\n"
                "  outputs      - Print masternode compatible outputs\n"
                "  start        - Start masternode configured in monkey.conf\n"
                ...
                "  scores       - Print list of winning masternode by score\n");
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6\""));
...
Value getmasternodecount (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() > 0))
        throw runtime_error(
            ...
            "  \"dscompat\": n,     (numeric) DarkSend Compatible\n"
    ...
    if (pindexBest)
        mnodeman.GetNextMasternodeInQueueForPayment(pindexBest->nHeight, true, nCount);

    obj.push_back(Pair("total", mnodeman.size()));
    obj.push_back(Pair("stable", mnodeman.stable_size()));
    obj.push_back(Pair("dscompat", mnodeman.CountEnabled(ActiveProtocol())));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) Monkey address for masternode payments\n"
