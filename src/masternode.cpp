...
void CMasternode::Check(bool forceCheck)
{
    ...
    if (!unitTest)
    {
        ...
        CTxOut vout = CTxOut((MASTERNODE_COLLATERAL - 0.01) * COIN, darkSendPool.collateralPubKey);
...

        {
            ...
            if (!AcceptableInputs(mempool, CTransaction(tx), false, NULL))
            {
...
bool CMasternodeBroadcast::Create(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& strErrorRet, CMasternodeBroadcast& mnbRet, bool fOffline)
{
    ...
    // int mainnetDefaultPort = Params(CChainParams::MAIN).GetDefaultPort();
...
bool CMasternodeBroadcast::CheckAndUpdate(int& nDos)
{
    ...
    // TODO:
    // if (Params().NetworkID() == CChainParams::MAIN)
    //     if (addr.GetPort() != 8710) return false;
    // } else if (addr.GetPort() == 8710)
    //     return false;
...
{
    vin = newVin;
    blockHash = FindBlockByHeight(nBestHeight - 12)->GetBlockHash();
    sigTime = GetAdjustedTime();
    vchSig = std::vector<unsigned char>();
}
...
bool CMasternodePing::CheckAndUpdate(int& nDos, bool fRequireEnabled)
{
    ...
    if (pmn != NULL && pmn->protocolVersion >= masternodePayments.GetMinMasternodePaymentsProto())
    {
        ...
        if (!pmn->IsPingedWithin(MASTERNODE_MIN_MNP_SECONDS - 60, sigTime))
        {
            ...
            std::map<uint256, CBlockIndex*>::const_iterator mi = mapBlockIndex.find(blockHash);
            if (mi != mapBlockIndex.end() && (*mi).second)
            {
