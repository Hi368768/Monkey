...
void CDarksendPool::ProcessMessageDarksend(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "dsq")
    {
        ...
        if (dsq.ready)
        {
            ...
            {
                LogPrintf("darksend", "Darksend queue is ready - %s\n", addr.ToString());
    ...
    else if (strCommand == "dssu")
    {
        ...
        if ((CNetAddr)pSubmittedToMasternode->addr != (CNetAddr)pfrom->addr){
            LogPrintf("darksend", "dssu - message doesn't match current Masternode - %s != %s\n", pSubmittedToMasternode->addr.ToString(), pfrom->addr.ToString());
            return;
        }
    ...
    else if (strCommand == "dsc")
    {
        ...
        if ((CNetAddr)pSubmittedToMasternode->addr != (CNetAddr)pfrom->addr)
        {
            LogPrintf("darksend", "dsc - message doesn't match current Masternode - %s != %s\n", pSubmittedToMasternode->addr.ToString().c_str(), pfrom->addr.ToString().c_str());
            return;
        }
...
std::string CDarksendPool::GetStatus()
{
    ...
    if (pindexBest->nHeight - cachedLastSuccess < minBlockSpacing ||!masternodeSync.IsBlockchainSynced())
    ...
    if (fMasterNode)
    {
        ...
        for (const CTransaction& txCollateral : vecSessionCollateral)
        {
            ...
            /*
                ...
                allow endless transaction that would bloat Monkey and make it unusable. To
                stop these kinds of attacks 1 in 50 successful transactions are charged. This
                adds up to a cost of 0.002 MONK per transaction on average.
            */
            if (r <= 10)
            {
...
bool CDarksendPool::SignFinalTransaction(CTransaction& finalTransactionNew, CNode* node)
{
    ...
    for (const CDarkSendEntry e : entries)
    {
        ...
        LogPrint("darksend", "CDarksendPool::Sign - txNew: %s\n", finalTransaction.ToString());
    }
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeMonkeyAmount*COIN - pwalletMain->GetAnonymizedBalance();
...
bool CDarksendQueue::Sign()
{
    ...
    {
        LogPrintf("CDarksendQueue::Sign() - ERROR: Invalid Masternodeprivkey: '%s'\n", errorMessage);
        return false;
    }
...
bool CDarksendQueue::Relay()
{
    ...
    for (CNode* pnode : vNodes)
        pnode->PushMessage("dsq", (*this)); // always relay to everyone
