...
//
// Bootup the masternode, look for MONK collateral input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if (Params().NetworkID() != CChainParams::REGTEST && !masternodeSync.IsBlockchainSynced()) {
    ...
    if (status != ACTIVE_MASTERNODE_STARTED) {
        ...
        //     if (service.GetPort() != 8710) {
        //         notCapableReason = strprintf("Invalid port: %u - only 8710 is supported on mainnet.", service.GetPort());
        //         LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //         return;
        //     }
        // } else if (service.GetPort() == 8710) {
        //     notCapableReason = strprintf("Invalid port: %u - 8710 is only supported on mainnet.", service.GetPort());
        //     LogPrintf("CActiveMasternode::ManageStatus() - not capable: %s\n", notCapableReason);
        //     return;
        // }
...
bool CActiveMasternode::Register(std::string strService, std::string strKeyMasternode, std::string strTxHash, std::string strOutputIndex, std::string& errorMessage)
{
    ...
    // if (Params().NetworkID() == CChainParams::MAIN) {
    //     if (service.GetPort() != 8710) {
    //         errorMessage = strprintf("Invalid port %u for masternode %s - only 8710 is supported on mainnet.", service.GetPort(), strService);
    //         LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //         return false;
    //     }
    // } else if (service.GetPort() == 8710) {
    //     errorMessage = strprintf("Invalid port %u for masternode %s - 8710 is only supported on mainnet.", service.GetPort(), strService);
    //     LogPrintf("CActiveMasternode::Register() - %s\n", errorMessage);
    //     return false;
    // }
    ...
    return Register(vin, CService(strService, true), keyCollateralAddress, pubKeyCollateralAddress, keyMasternode, pubKeyMasternode, errorMessage);
}
