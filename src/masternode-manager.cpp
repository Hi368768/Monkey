...
int CMasternodeMan::stable_size()
{
    ...
    int64_t nMasternode_Min_Age = GetSporkValue(SPORK_11_MN_WINNER_MINIMUM_AGE);
    ...
    if (strCommand == "mnb") { //Masternode Broadcast
        ...
        if (pmn != NULL) {
            ...
            if (count == -1 && pmn->pubKeyCollateralAddress == pubkey && (GetAdjustedTime() - pmn->nLastDsee > MASTERNODE_MIN_MNB_SECONDS)) {
                ...
                //     return;
                if (sigTime - pmn->lastPing.sigTime < MASTERNODE_MIN_MNB_SECONDS)
                    return;
        ...
        bool fAcceptable = false;
        {
            ...
            if (!lockMain) return;
            fAcceptable = AcceptableInputs(mempool, CTransaction(tx), false, NULL);
        }
