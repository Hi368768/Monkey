...
bool IsBlockValueValid(const CBlock& block, CAmount nExpectedValue, CAmount nMinted)
{
    ...
    if (pindexPrev->GetBlockHash() == block.hashPrevBlock)
        ...
        std::map<uint256, CBlockIndex*>::const_iterator mi = mapBlockIndex.find(block.hashPrevBlock);
...
{
    // there is no data to use to check anything -- find the longest chain
    if (!masternodeSync.IsSynced()) {
        LogPrint("mnpayments", "Client not synced, skipping block payee checks\n");
        return true;
    }

    const CTransaction& txNew = (nBlockHeight > Params().EndPoWBlock() ? block.vtx[1] : block.vtx[0]);
...
void CMasternodePayments::FillBlockPayee(CTransaction& txNew, int64_t nFees, bool fProofOfStake)
{
    ...
    CAmount blockValue = GetProofOfStakeReward(pindexPrev->nHeight, 0, 0);
    ...
    if (hasPayment) {
        if (fProofOfStake) {
            ...
            // if (hasPayment && txNew.vout.size() == 3)
...
int CMasternodePayments::GetMinMasternodePaymentsProto()
{
    if (IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES))
        ...
        return MIN_MASTERNODE_PAYMENT_PROTO_VERSION;      // Also allow old peers as long as they are allowed to run
}
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    CAmount nReward = GetProofOfStakeReward(nBlockHeight, 0, 0);
